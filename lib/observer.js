...
const createNode = (Obr, Ob, unit, init, fn) => {
  const node = ({ parent, arr, root = false }) => {
    if (arr.length === 0) {
      if (root) {
        events.subscribe(Ob, () => {
          events.publish(parent, unit);
      ...
      const [first] = arr;
      events.subscribe(Ob, () => {
        events.publish(parent, fn(unit, first));
      });
    } else if (arr.length === 2) {
      const [first, second] = arr;
      events.subscribe(Ob, () => {
        events.publish(parent, fn(first, second));
      });
    } else {
      const [first, ...rest] = arr;
      let acc = null;
      const p = uuid();
      events.create(p);
      events.subscribe(p, v => {
        if (acc) events.publish(parent, fn(acc, v));
        else acc = fn(first, v);
      });
      const half =
        rest.length % 2 === 0 ? rest.length / 2 : (rest.length - 1) / 2;
      node({ parent: p, arr: rest.slice(half) }); //left
      node({ parent: p, arr: rest.slice(0, half) }); //right
    }
  };

  node({ parent: Obr, arr: init, root: true });
};
